...
| 16 | Dọn nốt phần hoãn (an toàn): page_size cap + dedup ownership + reconcile requirements | cleanup | low | med | M | low | 13 | ✅ 2026-06-29 nhánh refactor/p16-cleanup. (1) companies page_size `Query(50,ge=1,le=100)` chặn query vô hạn; (2) dedup ~19 inline `select(Site)` ownership → `verify_site_access` (exports/segments/campaigns/sites) + xóa import Site chết; (3) xóa `apps/api/requirements.txt` cũ/không-dùng (cruft sendgrid/geoip2/pyjwt app không import; root là nguồn sự thật Docker+CI). BỎ: P10 concurrency + api.ts 401 dedup + worker.js (đổi hành vi/điều tra). 353 unit + integration 569 pass (4 fail PRE-EXISTING). |
