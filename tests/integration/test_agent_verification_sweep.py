...
async def test_sweep_upgrades_only_matching_openai_row(test_db, monkeypatch):
    ...
    # Tokens are stored exactly as classify_agent emits them: lower-cased, drawn
    # from _VENDOR_TOKENS. The sweep's SQL filter is a case-sensitive IN against
    # that same lower-case set, so a differently-cased fixture would not be
    # selected at all and the test would prove nothing about verification.
    #
    # Matching openai row (IP in mock 10.99.0.0/16) → should upgrade.
    ...
    # The three verdicts, and the distinction between the last two is the point.
    assert await _method(match.id) == "ip-verified"
    # Publishes ranges, arrived from outside all of them — the shape of a forged
    # UA. NOT "ua-only": that would say "we could not check", when we did.
    assert await _method(no_match.id) == "ip-mismatch"
    # Anthropic publishes nothing, so no conclusion is possible and the row must
    # stay at the ua-only ceiling. Never downgraded to a mismatch — absence of
    # published ranges is not evidence of forgery.
