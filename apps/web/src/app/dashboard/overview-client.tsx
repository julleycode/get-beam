...
function SiteCard({ site }: { site: Site }) {
  ...
  // Shared cache key with TodayActions (queryClient.fetchQuery) and seeded by
  // the dashboard-overview prefetch — one stats request per site, cached.
...
export default function OverviewClient() {
  ...
  // Sites + per-site stats in one query. Server-prefetched into this exact key
  // by page.tsx (HydrationBoundary), so on first load the data is already here
  // — no waiting on Clerk's client token before the Overview can paint. Falls
  // back to listSites if the aggregate endpoint is unavailable.
  const {
    data: overview,
    isError,
    refetch,
    ...
    queryFn: async (): Promise<{ sites: Site[]; stats: Record<string, SiteStats> }> => {
      try {
  ...
  // Seed per-site stats so SiteCard + TodayActions read cache (no refetch), and
  // bounce to onboarding when the account has no sites yet.
  useEffect(() => {
    if (!overview) return;
    for (const [siteId, stats] of Object.entries(overview.stats)) {
      qc.setQueryData(["visitor-stats", siteId], stats);
    }
    if (overview.sites.length === 0) router.push("/dashboard/onboarding");
  }, [overview, qc, router]);

  if (isError) {
    return (
      <ErrorBanner
        message="Couldn't load your sites — please try again."
        onRetry={() => refetch()}
      />
    );
  }
