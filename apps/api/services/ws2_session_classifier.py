...
This module looks at WITHIN-SESSION BEHAVIOR instead, in two stages:
    ...
    Stage 1 — deterministic fast-path. navigator.webdriver / a HeadlessChrome
              ...
              present). NOTE: in real production traffic these are ~never present
              because tracker.js line 4 already early-returns on
              navigator.webdriver — so this stage is defense-in-depth, proven at
              the unit tier only (plan Concern C2), never end-to-end.
