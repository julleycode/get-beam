...
function groupOf(relPath) {
  ...
  // Normalize to `/` so Windows `path.sep` (`\`) does not collapse `planning/all-*.md` to a single segment.
  const normalized = String(relPath).replace(/\\/g, "/");
  const rest = normalized.replace(/^process\/context\//, "");
...
function groupEntrypoints() {
  return contextDocsWithFm()
    .filter((d) => {
      const posix = String(d.path).replace(/\\/g, "/");
      // Exclude root router `process/context/all-context.md`; keep group `all-*.md` only.
      return /\/all-[^/]+\.md$/.test(posix) && groupOf(posix);
    })
...
function buildRoutingBlock() {
  ...
  const rootRows = [
    ...
    ...eps.map((d) => `| \`${toPosix(d.path)}\` | ${d.fm.readWhen || d.fm.description || "—"} |`),
  ];
  const groupRows = eps.length
    ? eps.map((d) => `| \`${groupOf(d.path)}/\` | \`${toPosix(d.path)}\` | ${d.fm.description || "—"} |`)
